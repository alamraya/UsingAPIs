...
function request($url, $token = null, $data = null, $pin = null){
    ...
    $data = request("","8a9b5eb3178ada6c4b069bc40806a5e0","?count=10&query=$keyword");
    ?>
...
<body>
    <nav class="navbar navbar-expand-lg navbar-dark bg-dark">
        ...
        <a class="navbar-brand" href="#">JURNAL</a>
            ...
            foreach ($data["search-results"]["entry"] as $row) : ?>
                <div class="col-md-12 mb-3">
                    <div class="card">
                        ...
                        <div class="card-body">
                            <h5 class="card-title"><?=$row["dc:title"]?></a></h5>
                            <p class="card-text">Creator: <?=$row["dc:creator"]?>
                            <p class="card-text">Volume: <?=$row["prism:volume"]?></p>
                        </div>
                    </div>
                </div>
           <?php foreach ($row["link"] as $row1) : ?>
            <button class="btn btn-outline-success my-2 my-sm-0" type=""><a href="<?= $row1["@href"]; ?>" target="_blank">Go</a></button>
